  ...
  return (
    <>
      ...
      <div className="box">
        ...
        <div className="tabs">
          <button className="tab active" id="tab-login">Sign In</button>
          <button className="tab" id="tab-reg">Register</button>
        </div>
        ...
        <div id="form-login">
          ...
          <button className="btn" id="l-btn">Sign In →</button>
        </div>
        <div id="form-reg" style={{display:'none'}}>
          ...
          <button className="btn" id="r-btn">Create Account →</button>
        </div>
      </div>
      <script dangerouslySetInnerHTML={{__html:`
        // Check if already logged in
        if (localStorage.getItem('rs_token')) window.location.href = '/dashboard';

        document.getElementById('tab-login').onclick = () => switchTab('login');
        document.getElementById('tab-reg').onclick = () => switchTab('register');
        ...
        function showErr(msg){const e=document.getElementById('err');e.textContent=msg;e.classList.add('show');document.getElementById('ok').classList.remove('show');}
        function showOk(msg){const e=document.getElementById('ok');e.textContent=msg;e.classList.add('show');document.getElementById('err').classList.remove('show');}
        ...
        async function doLogin() {
          ...
          try {
            ...
            if(!r.ok){showErr(d.error||'Login failed.');btn.disabled=false;btn.textContent='Sign In →';return;}
            localStorage.setItem('rs_token', d.token);
            localStorage.setItem('rs_email', email);
            window.location.href='/dashboard';
          } catch(e){showErr('Network error.');btn.disabled=false;btn.textContent='Sign In →';}
        }
        ...
        async function doRegister() {
          ...
          try {
            ...
            const d=await r.json();
            if(!r.ok){showErr(d.error||'Registration failed.');btn.disabled=false;btn.textContent='Create Account →';return;}
            localStorage.setItem('rs_token', d.token);
            localStorage.setItem('rs_email', email);
            showOk('Account created! Redirecting...');
            setTimeout(()=>window.location.href='/dashboard', 800);
          } catch(e){showErr('Network error.');btn.disabled=false;btn.textContent='Create Account →';}
        }
